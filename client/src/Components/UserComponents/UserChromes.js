...
const UserChromes = () => {
  ...
  const handleNewChrome = async (chromeId) => {
    ...
    await fetch(
      `${API}/users/add-chrome/${user.id}/${chromeId}`,
      requestOptions
    )
      .then(async (res) => {
        const data = await res.json();
  ...
  const handleRemoveChrome = (chromeId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        fetch(
          `${API}/users/remove-chrome/${user.id}/${chromeId}`,
          requestOptions
        )
          .then(async (res) => {
            const data = await res.json();
        ...
        Swal.fire(
          "Eliminada!",
          "Una figurita ha sido descontada de tu lista.",
          "success"
        );
      } else {
        ...
        {!chromes || user === undefined ? (
          ...
          <>
            <div className="pt-3">
              <section
                id="fwc-1"
                className="bg-qatar bg-team rounded mt-3 shadow"
              >
                <div className="row g-2">
                  <div className="d-flex flex-column position-relative">
                    <hr
                      className="text-light mt-3 w-100 position-absolute"
                      style={{ borderTop: "4px solid #f8f9fa" }}
                    />
                  ...
                  {chromes
                    ?.filter((chrome) => chrome.section === "FWC1")
                    .map((chrome, i) => {
                      return (
                        i < 19 && (
                            ...
                            >
                              {userChromes?.includes(chrome._id) ? (
                                <div className="d-flex flex-column h-100">
                                  {repeatedChromes?.map((repeat, i) => {
                                    return (
                                      repeat.chrome === chrome._id && (
                                        <div
                                          key={i}
                                          className="bg-warning text-center fw-bold rounded-top"
                                        >
                                          REPETIDA
                                        </div>
                                      )
                                  ...
                                  {repeatedChromes?.filter(
                                    (rep) => rep.chrome === chrome._id
                                  ).length > 0 ? (
                                    <>
                                      {repeatedChromes.map((repeat, i) => {
                                        return (
                                          repeat.chrome === chrome._id && (
                                            <div
                                              key={i}
                                              className="d-flex bg-warning text-center align-items-center rounded-bottom"
                                            >
                                              {clicked === chrome._id ? (
                                                <button
                                                  className={`btn btn-sm btn-qatar rounded-bottom w-100 ${
                                                    clicked === chrome._id &&
                                                    "disabled"
                                                  }`}
                                                >
                                                ...
                                                <>
                                                  <button
                                                    ...
                                                    className="btn btn-sm btn-qatar chrome-down"
                                                  >
                                                    -1
                                                  </button>
                                                  ...
                                                  <button
                                                    ...
                                                    className="btn btn-sm btn-qatar chrome-up"
                                                  >
                                                    +1
                                                  </button>
                                                </>
            ...
            {TEAMS.map((team, i) => (
              <div key={i} className="pt-3">
                <section
                  ...
                  className={`bg-${team
                    .replace(/\s+/g, "-")
                    .toLocaleLowerCase()} bg-team rounded shadow`}
                >
                  <div className="row g-2">
                    <div className="d-flex flex-column position-relative">
                      <hr
                            ...
                            {userChromes?.includes(chrome._id) ? (
                              <div className="d-flex flex-column h-100">
                                ...
                                {repeatedChromes?.filter(
                                  ...
                                  <button
                                    onClick={() => handleNewChrome(chrome._id)}
                                    className={`btn btn-sm btn-qatar rounded-bottom w-100 ${
                                      clicked === chrome._id && "disabled"
                                    }`}
                                  >
            ...
            <div className="py-3">
              <section
                id="fwc-2"
                className="bg-qatar bg-team rounded shadow pb-3"
              >
                <div className="row g-2">
                  <div className="d-flex flex-column position-relative">
                    <hr
                  ...
                  {chromes
                    ?.filter((chrome) => chrome.section === "FWC2")
                    .map((chrome, i) => (
